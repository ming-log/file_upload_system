...
def _generate_object_key(object_name: str) -> str:
...
class MinioStorageService:
    ...
    def save(
        ...
        """
        object_key = _generate_object_key(object_name)
        try:
...
class FakeStorageService:
    ...
    def save(
        ...
        if self.fail:
            return StorageResult.failed()
        storage_id = _generate_object_key(object_name)
...
class LocalDiskStorageService:
    ...
    def path_for(self, storage_id: str) -> str:
        ...
        # 防御路径穿越：仅取文件名部分。
        safe = os.path.basename(storage_id)
    ...
    def save(
        ...
        """将字节内容写入磁盘，成功返回唯一 ``storage_id``。"""
        storage_id = _generate_object_key(object_name)
        try:
            with open(self.path_for(storage_id), "wb") as fh:
                fh.write(data)
        except Exception:
            return StorageResult.failed()
        return StorageResult.success(storage_id)
